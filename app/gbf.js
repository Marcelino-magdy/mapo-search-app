...
const romania_graph = {
    ...
    function h_Cost( a, b ) {
      ...
      let city_a = a[a.length - 1]["city"] ;
      let h_cost_a = H_table[city_a];
      
      let city_b = b[b.length - 1]["city"] ;
      let h_cost_b = H_table[city_b];

      if ( h_cost_a < h_cost_b ){
        return -1;
      }
      if ( h_cost_a > h_cost_b ){
        return 1;
...
const Home = () =>{
    ...
    const search_result = GBF(romania_graph,rootCity,goalCity);
